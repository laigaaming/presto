...
public class PrestoBatchWriter
{
            ...
            throws MutationsRejectedException, TableNotFoundException
    {
        ...
        for (Map.Entry<Triple<String, String, ColumnVisibility>, Object> entry : columnUpdates.entrySet()) {
            deleteMutation.putDelete(entry.getKey().getLeft(), entry.getKey().getMiddle(), entry.getKey().getRight(), deleteTimestamp);
        }
        ...
        for (Map.Entry<Triple<String, String, ColumnVisibility>, Object> entry : columnUpdates.entrySet()) {
            ...
            if (Types.isArrayType(type)) {
                ...
                for (Object element : (List<?>) entry.getValue()) {
                    Value value = new Value(setText(elementType, element, text, serializer).copyBytes());
            ...
            throws MutationsRejectedException, TableNotFoundException
    {
        ...
        dataWriter.addMutation(toDeleteMutation(rowBytes, rowIdOrdinal, columns));
        if (indexer.isPresent()) {
            ...
            throws MutationsRejectedException, TableNotFoundException
    {
        ...
        rowIds.forEach(rowId -> {
            ...
            mutationBuilder.add(toDeleteMutation(text.copyBytes(), rowIdOrdinal, columns));
        });
        List<Mutation> mutations = mutationBuilder.build();

        // Write the mutation and index it
        dataWriter.addMutations(mutations);
        if (indexer.isPresent()) {
            indexer.get().delete(auths, mutations.stream().map(Mutation::getRow).collect(Collectors.toList()));
        }
    }
    ...
    /**
     * Converts a {@link Row} to a delete Accumulo mutation.
     *
     * @param rowId Bytes of the row ID
     * @param rowIdOrdinal Ordinal in the list of columns that is the row ID. This isn't checked at all, so I hope you're right. Also, it is expected that the list of column handles is sorted in ordinal order. This is a very demanding function.
     * @param columns All column handles for the Row, sorted by ordinal.
     * * @return Mutation
     */
    public static Mutation toDeleteMutation(byte[] rowId, int rowIdOrdinal, List<AccumuloColumnHandle> columns)
    {
        // Delete row ID from special column
        Mutation mutation = new Mutation(rowId);
        mutation.putDelete(ROW_ID_COLUMN, ROW_ID_COLUMN);

        // Iterate through all the column handles (minus the row ID column) and delete the columns
        for (AccumuloColumnHandle columnHandle : columns) {
            if (columnHandle.getFamily().isPresent() && columnHandle.getQualifier().isPresent()) {
                mutation.putDelete(columnHandle.getFamily().get(), columnHandle.getQualifier().get());
            }
        }
