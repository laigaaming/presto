...
public class ColumnCardinalityCache
{
            ...
            throws ExecutionException
    {
        ...
        Collection<CacheKey> exactRanges = colValues.stream()
                .filter(this::isExact)
                .map(range -> new CacheKey(schema, table, family, qualifier, range))
    ...
    {
        return !r.isInfiniteStartKey() && !r.isInfiniteStopKey() && r.getStartKey().followingKey(PartialKey.ROW).equals(r.getEndKey());
    }
            ...
            extends CacheLoader<CacheKey, Long>
    {
                ...
                throws Exception
        {
            ...
            Text columnFamily = new Text(getIndexColumnFamily(key.family.getBytes(UTF_8), key.qualifier.getBytes(UTF_8)).array());
            ...
            try {
                ...
                for (Entry<Key, Value> entry : scanner) {
                    numEntries += parseLong(entry.getValue().toString());
                }
                return numEntries;
            }
            finally {
                ...
                throws Exception
        {
            ...
            try {
                ...
                bScanner.fetchColumn(columnFamily, CARDINALITY_CQ_AS_TEXT);
