            ...
            throws MutationsRejectedException, TableNotFoundException
    {
        ...
        try {
            ...
            for (Entry<Key, Value> entry : scanner) {
                String family = entry.getKey().getColumnFamily(familyText).toString();
                String qualifier = entry.getKey().getColumnQualifier(qualifierText).toString();
                ColumnVisibility visibility = new ColumnVisibility(entry.getKey().getColumnVisibility(visibilityText).copyBytes());

                // If this current entry does not match a column we are updating, then continue to the next entry
                if (!columnUpdates.keySet().stream().filter(triple -> triple.getLeft().equals(family) && triple.getMiddle().equals(qualifier)).findAny().isPresent()) {
                    continue;
                }

                ByteBuffer familyBytes = wrap(familyText.copyBytes());
                ByteBuffer qualifierBytes = wrap(qualifierText.copyBytes());
