...
local schema = ARGV[1]
local table = ARGV[2]
local column_family = ARGV[3]
local auths = split(ARGV[4], ',')
local min = ARGV[5]
local max = ARGV[6]
...
for i, key in ipairs(keys) do
    local visibility = get_visibility(key)
    if visibility:len() == 0 or evaluate(auths, visibility, get_parse_tree(visibility)) then
        local hash_key = get_hash_key_string(schema, table, column_family, visibility)
        local val = redis.call('zrangebylex', key, min, max)
        for _, v in ipairs(val) do
            local hash_value = redis.call('hget', hash_key, v)
            if hash_value then sum = sum + tonumber(hash_value) end
        end
    end
end
